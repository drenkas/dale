...
|---|---|---|---|
| Luna standard | `gpt-5.6-luna` | Mechanical extraction, narrow checks, fresh low-risk reviewer lenses | `low` to `medium` |
| Luna deep | `gpt-5.6-luna` | Everyday implementation, debugging, integration, ordinary review | `xhigh` to `max` |
| Sol | `gpt-5.6-sol` | Consequential ambiguity, difficult cross-layer synthesis, adversarial proof | `high` |
...
2. Use Luna for bounded mechanical work whose correctness does not depend on
   deeper synthesis.
3. Use Luna with `xhigh` or `max` reasoning for substantial implementation or
   coupled reasoning.
...
Typical starting routes are Luna `xhigh` for the resident Worker, Sol `high`
...
- Use `max` when the owned reasoning bottleneck or proof obligation genuinely
  ...
  Sol and only when the live schema confirms that exact combination.
